...
# 🔧 Помощники для деградирующего режима, если ИИ временно недоступен
def _generate_fallback_advice_single(occasion: str, preferences: str) -> str:
    try:
        base = [
            "Пока сервис анализа недоступен, вот общий совет стилиста:",
            "1) Выбирайте силуэт, подчёркивающий вашу фигуру.",
            "2) Следите за сочетанием фактур: мягкие ткани + одна акцентная вещь.",
            "3) Добавьте аксессуар с лёгким блеском (золото/серебро) для акцента.",
            "4) Балансируйте пропорции: объём сверху — более спокойный низ и наоборот.",
        ]
        extra = []
        if occasion:
            extra.append(f"Повод: {occasion} — выбирайте уместные по дресс-коду материалы и длину.")
        if preferences:
            extra.append(f"Ваш запрос: {preferences} — учитывайте это при выборе цвета и аксессуаров.")
        return "\n".join(base + extra)
    except Exception:
        return "Сервис анализа временно недоступен. Рекомендуем выбрать лаконичный силуэт, уместные материалы под повод и 1–2 акцентных детали."

def _generate_fallback_advice_compare(occasion: str, preferences: str, images_count: int) -> str:
    try:
        header = "Пока сервис сравнения недоступен, общий совет по выбору лучшего образа:"
        lines = [
            "1) Оцените посадку и пропорции: что визуально стройнит и не стесняет движения.",
            "2) Смотрите на гармонию цвета с оттенком кожи и волос.",
            "3) Сравните качество ткани и деталей отделки.",
            "4) Выберите образ, который уверенно поддерживает заявленный повод.",
        ]
        tail = []
        if occasion:
            tail.append(f"Повод: {occasion} — отдайте приоритет уместности и комфорту.")
        if preferences:
            tail.append(f"Ваши пожелания: {preferences} — ориентируйтесь на них при выборе.")
        return "\n".join([header] + lines + tail + [f"Количество образов для сравнения: {images_count}"])
    except Exception:
        return "Сервис сравнения временно недоступен. Сфокусируйтесь на посадке, пропорциях и уместности под повод."
...
async def analyze_consultation(request: Request):
    ...
    try:
        ...
        try:
            ...
            # Вместо 500 — отдаём деградирующий ответ с нулевой стоимостью
            ...
            return {
                ...
                "advice": _generate_fallback_advice_single(occasion, preferences),
                "balance": current_balance,
                "cost": 0,
                "correlation_id": correlation_id,
                "processing_time": round(processing_time, 2),
                "status": "degraded",
                "note": "Gemini unavailable, returned fallback advice"
            }
