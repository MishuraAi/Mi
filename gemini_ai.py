...
ВЕРСИЯ: 0.3.1.2 (исправлены импорты asyncio и ImageDraw)
ФАЙЛ: gemini_ai.py
...
from PIL import Image, ImageOps, ImageDraw # <--- ДОБАВЛЕНО ImageDraw
...
async def test_gemini_connection():
    ...
    try:
        ...
        response = await model.generate_content_async("Test connection to Gemini API")
...
def handle_gemini_error(error, context_message=""):
    ...
    if "api key" in error_str or "authentication" in error_str:
        ...
        return f"Ошибка при обработке изображения ({context_message}): {str(error)[:150]}"
...
def optimize_image(img: Image.Image, max_size=1024, quality=85) -> bytes:
    ...
    try:
        ...
        if img.mode in ('RGBA', 'LA') or (img.mode == 'P' and 'transparency' in img.info):
            ...
            img_rgba_for_paste = img.convert("RGBA")
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            if full_response_text.strip():
                ...
                setattr(last_error, 'response', response)
                if finish_reason_str in ["SAFETY", "RECITATION", "MAX_TOKENS"]:
...
async def analyze_clothing_image(image_data: bytes, occasion: str, preferences: str = None):
    ...
    try:
        ...
        optimized_image_bytes = optimize_image(pil_image, quality=85)
    ...
    if CACHE_ENABLED and not _is_error_message(response_text):
        cache_manager.save_to_cache(image_data, occasion, response_text, preferences)
...
async def compare_clothing_images(image_data_list: list[bytes], occasion: str, preferences: str = None):
    ...
    if not (2 <= len(image_data_list) <= 5): # В ТЗ было до 5, в HTML слотов 4. Оставляю 5 как максимум API.
...
if __name__ == "__main__":
    async def main_test():
        print(f"Тестирование модуля Gemini AI для проекта МИШУРА (v0.3.1.2)... Кэш {'ВКЛЮЧЕН' if CACHE_ENABLED else 'ОТКЛЮЧЕН'}.")
        if not API_CONFIGURED_SUCCESSFULLY:
        ...
        def get_test_image_bytes(color_name: str, text_on_img: str, rgb_tuple: tuple, quality: int = 75, size: tuple = (300,400)) -> bytes:
            ...
            draw = ImageDraw.Draw(img) # Используем импортированный ImageDraw
            try:
                from PIL import ImageFont # ImageFont используется только здесь, можно оставить локальный импорт
                font_size = int(size[1] / 10)
                try: font = ImageFont.truetype("arial.ttf", font_size)
                except IOError:
                    try: font = ImageFont.truetype("DejaVuSans.ttf", font_size)
                    except IOError: font = ImageFont.load_default()
                draw.text((img.width*0.1, img.height*0.45), text_on_img, fill="white", font=font)
