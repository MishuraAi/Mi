...
ВЕРСИЯ: 0.4.0 (ИСПРАВЛЕНО ПОДКЛЮЧЕНИЕ К API)
ДАТА ОБНОВЛЕНИЯ: 2025-06-19

ИСПРАВЛЕНИЯ В ВЕРСИИ 0.4.0:
- Исправлено подключение к Gemini API (правильная конфигурация)
- Убраны асинхронные вызовы в test_gemini_connection для стабильности
- Упрощена обработка ошибок
- Добавлена детальная диагностика подключения
- Исправлена модель на gemini-1.5-flash-latest
==========================================================================================
"""
import os
import logging
...
def create_analysis_prompt(occasion: str, preferences: Optional[str] = None) -> str:
    ...
    base_prompt = f"""Ты - профессиональный стилист МИШУРА. Проанализируй одежду на изображении для {occasion}.

Оцени:
1. Общий стиль и уместность для указанного повода
2. Цветовую гамму и сочетание цветов  
3. Соответствие образа
...
В конце добавь практический совет для будущих консультаций, начиная со слов "💡 Совет от МИШУРЫ:"

Ответь на русском языке в дружелюбном тоне."""
...
def create_comparison_prompt(occasion: str, preferences: Optional[str] = None) -> str:
    """Создает промпт для сравнения образов."""
    base_prompt = f"""Ты - профессиональный стилист МИШУРА. Сравни представленные образы для {occasion}.

Для каждого образа оцени:
1. Уместность для повода
2. Цветовую гамму
3. Общую гармонию образа
4. Практичность

В конце:
- Выбери лучший образ и объясни почему
- Дай рекомендации по улучшению каждого
- Добавь совет для будущих сравнений: "💡 Совет от МИШУРЫ:"
